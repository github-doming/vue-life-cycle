<template>
  <div>
    <a-table :columns="columns" :data-source="dataSource" :scroll="{x: stageData.length * 50 + 100,y:300}"
      ...
      <a-checkbox @change="cellClick(state)" slot="checkbox" v-model="state.checked" slot-scope="state"/>
    </a-table>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      cellClick(data) {
        let transforms = this.currentStage.transform;
        ...
        if (data.checked) {
          transforms.push({transition: data.transition, state: [data.state]});
        }
      },
