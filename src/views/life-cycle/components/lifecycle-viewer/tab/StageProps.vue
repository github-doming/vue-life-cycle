<template>
  <div class="props-context">
    <div class="props-status">
      ...
      <a-select class="select-wrap" v-model="currentStage.code" :placeholder="local.selectStatus" :open="false">
        <a-select-option v-for="item in states" :key="item.code">{{item.name}}</a-select-option>
      </a-select>
    </div>
    <div>
      <label>{{local.versionSeries}}：</label>
      <a-select class="select-wrap" v-model="currentStage.version" :placeholder="local.selectVersionSeries" :open="false">
